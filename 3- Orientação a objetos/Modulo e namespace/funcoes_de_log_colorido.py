...
def imprimir_no_log(texto, nivel='info'):
    if nivel.lower() == 'info':
        print(colorama.Fore.LIGHTBLUE_EX + f'Info: ', end='') # cor que vou escolher exemplo lightblue
        print(colorama.Style.RESET_ALL + texto)
    elif nivel.lower() == 'aviso':
        print(colorama.Fore.YELLOW + f'Aviso: ', end='') #for = cor escolhida no caso é verde
        print(colorama.Style.RESET_ALL + texto)
